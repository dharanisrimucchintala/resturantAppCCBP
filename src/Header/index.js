...
const Header = ({cartItems}) => {
  const [tabs, setTabs] = useState([]) // To store API response
  ...
  useEffect(() => {
    ...
    const fetchTabs = async () => {
      try {
        ...
        console.log(data) // Check what the structure is
        if (data.menu_tabs) {
          setTabs(data.menu_tabs) // Update tabs state with API response
        }
      } catch (error) {
  ...
  return (
    <header className="p-4 d-flex flex-row align-items-center nav-header">
      ...
      <ul className="tabs-list">{renderTabs()}</ul>
